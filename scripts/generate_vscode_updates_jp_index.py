#!/usr/bin/env python3
"""Generate an index for Japanese translations under vscode-updates-jp/.

Creates/updates:
- vscode-updates-jp/index.md
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate index for JP VS Code Updates translations"
    )
    parser.add_argument(
        "--jp-dir",
    ...
    lines = ["# VS Code Updates Archive (JP)", "", "## Entries", ""]
    ...
    print(
        f"VS Code Updates JP index updated: {os.path.relpath(index_path, os.getcwd())} (entries: {len(items)})"
    )
    return 0
