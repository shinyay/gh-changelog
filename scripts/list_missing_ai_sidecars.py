#!/usr/bin/env python3
"""List Markdown entries that are missing AI sidecar JSON files.

This supports the "Copilot Chat運用" workflow:
1) Run scripts/fetch_changelog.py to generate/update Markdown.
2) Use Copilot Chat with the prompt files under .github/prompts/ to generate JSON.
3) Save JSON to changelogs/_ai/<same-stem>.ai.json.
4) Run scripts/validate_ai_outputs.py.

This script helps you find which entries still need a sidecar.
"""
...
if __name__ == "__main__":
    try:
        ...
        # Allow piping to tools like `head` without a stack trace.
        raise SystemExit(0)
