#!/usr/bin/env python3
"""List English VS Code Updates Markdown entries that are missing Japanese translation files.

Mapping:
- EN: vscode-updates/<stem>.md (excluding index.md)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="List EN VS Code Updates Markdown files missing JP translations"
    )
    parser.add_argument(
        "--en-dir",
    ...
    for en_md, jp_md in missing:
        print(f"- {os.path.basename(en_md)} -> {os.path.relpath(jp_md, os.getcwd())}")
